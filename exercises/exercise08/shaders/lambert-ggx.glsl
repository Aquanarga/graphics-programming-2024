...
float DistributionGGX(vec3 normal, vec3 halfDir, float roughness)
{
	...
	float dot = ClampedDot(normal, halfDir);
	float expr = dot * dot * (alpha - 1.0f) + 1.0f;

	return alpha / (Pi * expr * expr);
}
...
float GeometrySchlickGGX(float cosAngle, float roughness)
{
	...
	return (2 * cosAngle) / (cosAngle + sqrt(alpha2 + (1 - alpha2) * cosAngle * cosAngle));
}
...
vec3 ComputeSpecularIndirectLighting(SurfaceData data, vec3 viewDir)
{
	...
	float geometry = GeometrySmith(data.normal, reflectionDir, viewDir, data.roughness);

	return specularLighting * geometry;
}
...
vec3 ComputeSpecularLighting(SurfaceData data, vec3 lightDir, vec3 viewDir)
{
	...
	float dots = ClampedDot(data.normal, viewDir) * ClampedDot(data.normal, lightDir);

	return vec3((d * g) / (4 * dots + 0.00001f));
}
